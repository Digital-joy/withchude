...
export default function Books({ document }: PropsWithChildren<{ document: PrismicDocument }>) {
  useEffect(() => {
    if (!document) return
                      ...
                      <div className="book" key={key}>
                        <div className="book__left">
                          ...
                          <button>
                            Get it on
                            {data.link1?.url && (
                              <a href={data.link1?.url} target="_blank" rel="noopener noreferrer">
                                {data["link1-logo"]?.url && <img src={data["link1-logo"]?.url?.split("?")[0]} alt="" />}
                              </a>
                            )}
                            {data.link2?.url && (
                              <a href={data.link2?.url} target="_blank" rel="noopener noreferrer">
                                {data["link2-logo"]?.url && <img src={data["link2-logo"]?.url?.split("?")[0]} alt="" />}
                              </a>
                            )}
                            {data.link3?.url && (
                              <a href={data.link3?.url} target="_blank" rel="noopener noreferrer">
                                {data["link3-logo"]?.url && <img src={data["link3-logo"]?.url?.split("?")[0]} alt="" />}
                              </a>
                            )}
                          </button>
                        </div>
