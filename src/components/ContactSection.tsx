...
export interface IContactSection extends React.ComponentPropsWithoutRef<'div'> {}

const ContactSection: React.FC<IContactSection> = ({ className, ...divProps }) => {
  useEffect(() => {
  ...
  return (
    <div className={`className="__contact" ${className}`} {...divProps}>
      ...
      )}
      {
        <div className="contact__body">
          <div className="contact__body__left">
            ...
            <p>Fill in the details in the contact form and a member of our team will respond to you shortly.</p>
            <i>Or send an e-mail</i>
            <a
              href="mailto:info@withchude.com"
              onClick={async (e) => {
                e.preventDefault()
                await navigator.clipboard.writeText("info@withchude.com")
                setCopy(true)
              }}
            >
              info@withchude.com
          ...
          <form className="contact__body__right" action="https://formspree.io/f/xovaqweq" onSubmit={handleSubmit}>
            {state.succeeded ? (
              <svg className="svg" width="1em" height="1em" viewBox="0 0 24 24">
                <g fill="none" stroke="currentColor" strokeWidth={1.5}>
                  <circle cx={12} cy={12} r={10}></circle>
              ...
              <>
                <label htmlFor="name">
                  Name
                  <div className="Signup__contact__show" onClick={() => setShow(!show)}></div>
                </label>{" "}
                <input
                  id="name"
                  name="name"
                ...
                <label htmlFor="message">
                  Message
                  <div className="Signup__contact__show" onClick={() => setShow2(!show2)}></div>
                </label>{" "}
                <textarea
                  id="message"
                  name="message"
                ...
                <button
                  ...
                  style={{
                    filter: `grayscale(${Name.length > 1 && Email.length > 5 && Message.length > 0 ? 0 : 1})`,
                    pointerEvents: Name.length > 1 && Email.length > 5 && Message.length > 0 ? "auto" : "none",
                  }}
                >
