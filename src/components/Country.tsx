    ...
    <aside className="Popup" onClick={Close}>
      ...
      >
        ...
        <section className="Signup__form">
          <h1>Select your country to proceed</h1>
          <form
            onSubmit={async (e) => {
              e.preventDefault()
              setLoading(true)
              const { data: res, error } = await supabase.auth.updateUser({
                data: { country: Value.value, countryCode: Value.value },
              })
              location.reload()
              setLoading(false)
            }}
          >
            ...
            <ReactSelect
              ...
              onChange={(selectedOption) => selectedOption && setValue(selectedOption)}
              required
            />
