...
import $Header from "../app/classes/header";
...
>([
  ...
  const [playing, setPlaying] = useState(filtered[0]);
    ...
    <>
      <section className="Hero">
        ...
        <div className="Hero__media__active">
          <video
            playsInline
            src={filtered && createSnippet(filtered[0].snippet, filtered[0].url)}
        ...
        {filtered.map((vid) => (
          <div className="Hero__media__active2" key={vid.uid}>
            ...
            <Link to={"/watch/" + vid.uid}>
              <PlaySvg />
              <span>Watch Full Episode</span>
            </Link>
          </div>
        ))}
          {/* <div className="Hero__media__active2" key={playing.uid}>
            <h2>
              <span style={{ textTransform: "uppercase" }}>{playing.episode}</span>
              {playing.episode !== "" && playing.title !== "" && ":"} {playing.title}
            </h2>
            <Link to={"/watch/" + playing.uid}>
              <PlaySvg />
              <span>Watch Full Episode</span>
            </Link>
          </div> */}
          ...
          {filtered.map((vid, index) => (
            <figure
              ...
              onClick={() => {
                ...
                // setPlaying(
                //   (({ uid, url, snippet, episode, title, thumbnail }) => ({
                //     uid,
                //     url,
                //     snippet,
                //     episode,
